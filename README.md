...
ボタンを出すかどうかは Square の SDK が判断します。対応していない場合（Wallet にカードが無い、その組み合わせを SDK がサポートしていない）は「使えない」と返るのでボタンは表示されません。Apple Pay 自体は Safari 以外のブラウザにも広がっていますが、Square の SDK がどこまで対応しているかは SDK 側の都合なので、こちらのコードでは制御していません。確実に出るのは iPhone / Mac の Safari です。読み込みが 3 秒で返らない場合も同じ扱いです。カード入力欄はその影響を受けません。
